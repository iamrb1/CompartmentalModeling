...
// Test 4: Using ComboManager with negative numbers
TEST_CASE("ComboManager: std::vector<int> with negative numbers",
          "[ComboManager]") {
...
// Test 5: Combination size 0 (should yield one empty combination).
...
// Test 6: Combination size equal to container size.
...
// Test 7: Combination size greater than container size.
TEST_CASE("ComboManager: Combination size greater than container size", "[ComboManager]") {
  std::vector<int> numbers = {1, 2, 3};
  // Invalid case: combination size greater than container size.
  cse::ComboManager<std::vector<int>> cm(numbers, 4);

  // Total combinations should be 0
  REQUIRE(cm.TotalCombinations() == 0);

  // No next combination should exist
  REQUIRE_FALSE(cm.NextCombo());

  // No previous combination should exist
...
// Test 9: Current indices correctness
...
// Test 10: Single element container
...
// Test 11: Empty container with combination size 0
...
// Test 13: Repeated calls to PrevCombo on first combination always return
// false.
TEST_CASE(
...
/**
 * Repeating Elements Tests
 */
TEST_CASE("ComboManager: Repeating Elements Without Required Element",
          "[ComboManager]") {
  std::vector<int> testVector = {1, 2, 3};
  // With repeats allowed and combination size 2, we should have C(3+2-1, 2) =
  // C(4,2) = 6 combinations. Expected combinations (non-decreasing order): {1,
  // 1}, {1, 2}, {1, 3}, {2, 2}, {2, 3}, {3, 3}
  cse::ComboManager<std::vector<int>> manager(testVector, 2, true);
  std::vector<std::vector<int>> correctCombos = {{1, 1}, {1, 2}, {1, 3},
                                                 {2, 2}, {2, 3}, {3, 3}};
...
TEST_CASE("ComboManager: Repeating Elements With Required Element",
          "[ComboManager]") {
  std::vector<int> testVector = {10, 20, 30, 40};
  // Require the element at index 2 (value 30) in every combination.
  // For combination size 3, the required mode removes the required element and
  // works with an effective size of 2. With repeats allowed, the available
...
TEST_CASE("ComboManager: Binomial Coefficient for Repeating Elements",
          "[ComboManager]") {
