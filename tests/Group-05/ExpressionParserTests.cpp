...
TEST_CASE("Parser Evaluations", "[parser]") {
  ...
  default_index = 0;
  func = parser.MakeFunc("{val3} + {val3} * {val2} ^ ({val2} + {val2})", 0,
                         default_index);
...
TEST_CASE("ExpressionParser constexprAdd", "[parser][constexpr]") {
  // This test will validate that the constexprAdd template function computes
  // the sum at compile time.
  constexpr double result =
      cse::ExpressionParser<std::map<std::string, double>>::constexprAdd(2.0,
                                                                         3.0);
  REQUIRE(result == 5.0);
}
...
TEST_CASE("ExpressionParser constexprSubtract", "[parser][constexpr]") {
  // This test will validate that the constexprSubtract template function
  // computes the difference at compile time.
  constexpr double result1 =
      cse::ExpressionParser<std::map<std::string, double>>::constexprSubtract(
          8.0, 3.0);
  REQUIRE(result1 == 5.0);
  constexpr double result2 =
      cse::ExpressionParser<std::map<std::string, double>>::constexprSubtract(
          2.0, 3.0);
  REQUIRE(result2 == -1.0);
}
...
TEST_CASE("ExpressionParser constexprMultiply", "[parser][constexpr]") {
  // This test will validate that the constexprMultiply template function
  // computes the product at compile time.
  constexpr double result1 =
      cse::ExpressionParser<std::map<std::string, double>>::constexprMultiply(
          8.0, 3.0);
...
TEST_CASE("ExpressionParser constexprDivide", "[parser][constexpr]") {
  // This test will validate that the constexprDivide template function computes
  // the quotient at compile time.
  constexpr double result1 =
      cse::ExpressionParser<std::map<std::string, double>>::constexprDivide(
          8.0, 2.0);
  REQUIRE(result1 == 4.0);
  constexpr double result2 =
      cse::ExpressionParser<std::map<std::string, double>>::constexprDivide(
...
TEST_CASE("ExpressionParser constexprPower", "[parser][constexpr]") {
  // This test will validate that the constexprPower template function computes
  // the power at compile time. Test positive exponents
  constexpr double result1 =
      cse::ExpressionParser<std::map<std::string, double>>::constexprPower(2.0,
                                                                           3);
  REQUIRE(result1 == 8.0);
  constexpr double result2 =
      cse::ExpressionParser<std::map<std::string, double>>::constexprPower(-2.0,
  ...
  constexpr double result3 =
      cse::ExpressionParser<std::map<std::string, double>>::constexprPower(5.0,
                                                                           0);
  REQUIRE(result3 == 1.0);
  constexpr double result4 =
      cse::ExpressionParser<std::map<std::string, double>>::constexprPower(-5.0,
                                                                           0);
  REQUIRE(result4 == -1.0);
  // Test negative exponents
  constexpr double result5 =
      cse::ExpressionParser<std::map<std::string, double>>::constexprPower(2.0,
                                                                           -2);
  REQUIRE(result5 == 0.25);
  constexpr double result6 =
      cse::ExpressionParser<std::map<std::string, double>>::constexprPower(-2.0,
                                                                           -2);
  REQUIRE(result6 == 0.25);
  constexpr double result7 =
      cse::ExpressionParser<std::map<std::string, double>>::constexprPower(-2.0,
                                                                           -3);
...
TEST_CASE("ExpressionParser constexprLessThan", "[parser][constexpr]") {
  // This test will validate that the constexprLessThan template function
  // computes the < comparison at compile time.
  constexpr bool result1 =
      cse::ExpressionParser<std::map<std::string, double>>::constexprLessThan(
          8.0, 3.0);
...
TEST_CASE("ExpressionParser constexprEqualTo", "[parser][constexpr]") {
  // This test will validate that the constexprEqualTo template function
  // computes the == comparison at compile time.
  constexpr bool result1 =
      cse::ExpressionParser<std::map<std::string, double>>::constexprEqualTo(
          8.0, 3.0);
  REQUIRE(result1 == false);
  constexpr bool result2 =
      cse::ExpressionParser<std::map<std::string, double>>::constexprEqualTo(
...
/* Verifies that the template function correctly creates a lambda that performs
  the binary operation (addition and subtraction) at compile time
*/
TEST_CASE("ExpressionParser MakeBinaryFun - Compile-time Binary Operation",
          "[parser][template]") {
  // Test compile-time addition:
  constexpr auto addFun =
      cse::ExpressionParser<std::map<std::string, double>>::MakeBinaryFun(
          2.0, 3.0, std::plus<>{});
  static_assert(addFun() == 5.0, "Compile-time addition failed");
  REQUIRE(addFun() == 5.0);

  // Test compile-time subtraction:
  constexpr auto subFun =
      cse::ExpressionParser<std::map<std::string, double>>::MakeBinaryFun(
          10.0, 4.0, std::minus<>{});
