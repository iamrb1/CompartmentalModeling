...
TEST_CASE("CSVFile LoadCsv - Valid File", "[CSVFile]") {
    ...
    std::tuple<const std::size_t, const std::size_t> grid_shape = data_grid.Shape();
...
TEST_CASE("CSVFile ExportCsv - Valid DataGrid", "[CSVFile]") {
    ...
    // Insert three rows for header and two data rows
    data_grid.InsertRow(0);
    data_grid.InsertRow(1);
    data_grid.InsertRow(2);

    // Fill header row
    {
        std::vector<cse::Datum> &row0 = data_grid.GetRow(0);
        row0.push_back(cse::Datum("Name"));
        row0.push_back(cse::Datum("Age"));
        row0.push_back(cse::Datum("Score"));
    }
    // Fill first data row
    {
        std::vector<cse::Datum> &row1 = data_grid.GetRow(1);
        row1.push_back(cse::Datum("Charlie"));
        row1.push_back(cse::Datum(28));
        row1.push_back(cse::Datum(91.0));
    }
    // Fill second data row
    {
        std::vector<cse::Datum> &row2 = data_grid.GetRow(2);
        row2.push_back(cse::Datum("Diana"));
        row2.push_back(cse::Datum(32));
        row2.push_back(cse::Datum(85.5));
    }
    ...
    // Check header row (no sanitization needed if there are no delimiters or quotes)
