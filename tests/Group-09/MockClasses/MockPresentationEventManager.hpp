/**
 * @file MockPresentationEventManager.hpp
 *
 * @author Owen Haiar
 */
...
class MockPresentationManager; ///< Forward declaration so Events can store a pointer to the MockPresentationManager

using namespace cse;
using Event_T = Event<MockPresentationManager *, size_t>;
using PresentationFunction = std::function<void(MockPresentationManager *, size_t)>;

class PresentationEventManager {
	private:
		MockPresentationManager *_presentation_manager;
		...
		std::vector<EventQueue<MockPresentationManager *, size_t> > _queues; ///< Vector of EventQueues where _queues[x] corresponds to slide x
		EventQueue<MockPresentationManager *, size_t> _active_queue; ///< Copy of queue currently active, this is where events are removed from
		std::map<size_t, size_t> _id_to_slide; ///< EventID to Slide Num map, should speed up remove

		void _update_clock() { //emscripten baased clock
...
//			_last_recorded_time = current_time;
	...
	public:
		explicit PresentationEventManager(MockPresentationManager *layout_manager) : _presentation_manager(layout_manager) {
		}
