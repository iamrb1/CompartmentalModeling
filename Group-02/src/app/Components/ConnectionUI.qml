...
/**
 * UI component for the connection between compartments
 */
Item {
    id: connectionUI
    ...
    /// Check if compartment connects to itself

    /// hash to create uniqueID string for each connection to allow space in between connections
    /// generated by Claude
    /// The use of this is to create and offset to prevent overlapping bidirectional connections
    property string connectionId: connection ? (source ? source.symbol : "") + "->"
                                               + (target ? target.symbol : "") : ""
    property bool isOddHash: connectionId.length
                             > 0 ? (connectionId.charCodeAt(
                                        0) + connectionId.charCodeAt(
                                        connectionId.length - 1)) % 2 !== 0 : false
    property int connectionOffset: isOddHash ? -15 : 15

    /// Creates a line with an arrow coming out of it as the shape
    Canvas {
        id: lineRect
        ...
        onPaint: {
            ...
            const arrowX = width - arrowSize * 2 - 80
            // Offset from end
            ctx.beginPath()
        ...
        MouseArea {
            ...
            onClicked: {
                ...
                /// connects to sidebar and displays values if clicked
            }
        }
    }

    /// Creates a self loop structure if the source and target component are the same
    Canvas {
        id: selfLoopCanvas
        ...
        onPaint: {
            ...
            ctx.arcTo(startX + loopWidth, startY, startX + loopWidth,
                      startY + cornerRadius, cornerRadius)
            ctx.lineTo(startX + loopWidth, startY + loopHeight - cornerRadius)
            ctx.arcTo(startX + loopWidth, startY + loopHeight,
                      startX + loopWidth - cornerRadius, startY + loopHeight,
                      cornerRadius)
            ctx.lineTo(startX + cornerRadius, startY + loopHeight)
            ctx.arcTo(startX, startY + loopHeight, startX,
                      startY + loopHeight - cornerRadius, cornerRadius)
            ctx.lineTo(startX, startY + cornerRadius)
            ctx.arcTo(startX, startY, startX + cornerRadius, startY,
                      cornerRadius)
            ctx.stroke()
