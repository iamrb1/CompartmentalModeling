...
int MainFrame::Initialize(int argc, char *argv[])
{
    ...
    if (auto *saveDlg = root->findChild<QObject*>("saveFileDialog")) {
        QObject::connect(
            saveDlg, SIGNAL(accepted(QUrl)),
            this,   SLOT(saveSimulation(QUrl))
        );
    }
    if (auto *openDlg = root->findChild<QObject*>("openFileDialog")) {
        QObject::connect(
            openDlg, SIGNAL(accepted(QUrl)),
            this,   SLOT(loadSimulation(QUrl))
        );
    }
...
void MainFrame::save_simulation(const QUrl &fileUrl) const {
    m_simulation->save_xml(fileUrl.toString(QUrl::FullyEncoded).toStdString());
}

void MainFrame::load_simulation(const QUrl &fileUrl) const {
    m_simulation->load_xml(fileUrl.toString(QUrl::FullyEncoded).toStdString());
}
