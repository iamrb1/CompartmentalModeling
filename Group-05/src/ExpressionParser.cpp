...
std::string cse::ExpressionParser<SymbolTableType>::GetParenthContent(
 ...
 */
template <typename SymbolTableType>
std::function<cse::Datum(SymbolTableType &)>
cse::ExpressionParser<SymbolTableType>::ParseKey(const std::string &expression,
                                                 size_t &index) {
  ...
  while (index < expression.size() && expression.at(index) != '{' &&
         isdigit(expression.at(index)) == false &&
         expression.at(index) != '(') {
    index++;
  }
...
cse::ExpressionParser<SymbolTableType>::MakeAddFun(
  ...
  return [left, right](SymbolTableType &symbols) {
    if (left(symbols).IsString() || right(symbols).IsString() ||
        std::isnan(left(symbols).AsDouble()) ||
        std::isnan(right(symbols).AsDouble())) {
      return std::nan("");
    }
...
template <typename SymbolTableType>
std::function<Datum(SymbolTableType &)>
cse::ExpressionParser<SymbolTableType>::MakeSubFun(
    std::function<Datum(SymbolTableType &)> left,
    std::function<Datum(SymbolTableType &)> right) {
  // using map_t = std::map<std::string, double>;
  return [left, right](SymbolTableType &symbols) {
...
template <typename SymbolTableType>
std::function<Datum(SymbolTableType &)>
cse::ExpressionParser<SymbolTableType>::MakeMulFun(
    std::function<Datum(SymbolTableType &)> left,
    std::function<Datum(SymbolTableType &)> right) {
  // using map_t = std::map<std::string, double>;
  return [left, right](SymbolTableType &symbols) {
...
template <typename SymbolTableType>
std::function<Datum(SymbolTableType &)>
cse::ExpressionParser<SymbolTableType>::MakeCosFun(
...
template <typename SymbolTableType>
std::function<Datum(SymbolTableType &)>
cse::ExpressionParser<SymbolTableType>::MakeSinFun(
    std::function<Datum(SymbolTableType &)> expression_lambda) {
  return [expression_lambda](SymbolTableType &symbols) {
...
template <typename SymbolTableType>
std::function<Datum(SymbolTableType &)>
cse::ExpressionParser<SymbolTableType>::MakeParenthFun(
    std::function<Datum(SymbolTableType &)> expression_lambda) {
  return [expression_lambda](SymbolTableType &symbols) {
...
template <typename SymbolTableType>
std::function<Datum(SymbolTableType &)>
cse::ExpressionParser<SymbolTableType>::MakeExpoFun(
    std::function<Datum(SymbolTableType &)> left,
    std::function<Datum(SymbolTableType &)> right) {
  // using map_t = std::map<std::string, double>;
  return [left, right](SymbolTableType &symbols) {
...
template <typename SymbolTableType>
std::function<Datum(SymbolTableType &)>
cse::ExpressionParser<SymbolTableType>::MakeDivFun(
    std::function<Datum(SymbolTableType &)> left,
    std::function<Datum(SymbolTableType &)> right) {
  return [left, right](SymbolTableType &symbols) {
    if (left(symbols).IsString() || right(symbols).IsString() ||
...
/**
 * @brief Evaluates equation represented by expression and returns simplified
 * value as a double
 *
 * @param symbol_table_ Map with value and key that will be regerences in
 * expression
...
template <typename SymbolTableType>
std::function<Datum(SymbolTableType &)>
cse::ExpressionParser<SymbolTableType>::MakeFunc(const std::string &expression,
                                                 int precedence,
                                                 size_t &index) {
  ...
  while (index < expression.size()) {
    ...
    if (op == '+' || op == '-' || op == '*' || op == '/' || op == 'c' ||
      ...
      if (op == comparison_char) {
        ...
        l1 = MakeAddFun(l1, MakeFunc(expression, 2, index));  // Creates
                                                              // function
      }
      ...
      if (op == comparison_char) {
        ...
        l1 = MakeSubFun(l1, MakeFunc(expression, 2, index));  // Creates
                                                              // function
      }
      ...
      if (op == comparison_char) {
        ...
        l1 = MakeMulFun(l1, MakeFunc(expression, 3, index));  // Creates
                                                              // function
      }
      ...
      if (op == comparison_char) {
        ...
        l1 = MakeDivFun(l1, MakeFunc(expression, 3, index));  // Creates
                                                              // function
      }
      ...
      if (op == comparison_char) {
        ...
        l1 = MakeExpoFun(l1,
                         MakeFunc(expression, 4, index));  // Creates function
      }
    }
    if (op == 'c') {
      default_index = 0;
      l1 = MakeCosFun(
          MakeFunc(GetParenthContent(expression, index + 3), 0, default_index));
    ...
    if (op == '(') {
      default_index = 0;
      l1 = MakeParenthFun(
          MakeFunc(GetParenthContent(expression, index), 0, default_index));
      SkipParenthesis(index, expression);
    }
...
template <typename SymbolTableType>
size_t cse::ExpressionParser<SymbolTableType>::getToken(
    const std::string &expression, size_t &index) {
  ...
  for (; index < expression.size(); index++) {
    ...
    if (expression.substr(index, 4) == "cos(" ||
        expression.substr(index, 4) == "sin(") {
  ...
  return std::string::npos;  // std::string::npos is the largest possible value
                             // for size_t
}

template <typename SymbolTableType>
std::function<Datum(SymbolTableType &)>
cse::ExpressionParser<SymbolTableType>::KeyLambda(const Datum &key) {
  ...
  if constexpr (requires(SymbolTableType &s) { s[std::string{}]; }) {
    ...
    return [key](SymbolTableType &symbols) { return symbols[key.GetString()]; };
  } else if constexpr (requires(SymbolTableType &s) { s[int{}]; }) {
    // Index with a number (e.g., for vector)
    return [key](SymbolTableType &symbols) { return symbols[key.AsDouble()]; };
  } else {
...
template <typename SymbolTableType>
void cse::ExpressionParser<SymbolTableType>::SetSymbolTable(
    const std::string &key, const double &num) {
