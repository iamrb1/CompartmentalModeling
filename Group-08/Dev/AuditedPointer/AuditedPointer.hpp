...
public:
    ...
    /**
     * @brief Copy constructor - copies the pointer (shallow copy)
     * @param other Aptr to copy from
     */
    Aptr(const Aptr& other) : mPtr(other.mPtr) {}

    /**
     * @brief Copy assignment operator - copies the pointer (shallow copy)
     * @param other Aptr to copy from
...
struct LeakChecker
{
     ...
     * @return Const reference to map of active pointer IDs to their addresses
     */
    const std::unordered_map<int, const void*>& GetActiveAptrs() const
    {
        return activeAptrs;
    }

    /**
     * @brief Finds an Aptr by its ID in the active list of Aptrs
...
public:
     ...
     * @param other Aptr to copy from
     */
    Aptr(const Aptr& other) : mPtr(other.mPtr), mID(GetNextID())
    {
        if (mPtr)
        {
            leakChecker.AddAptr(mID, mPtr);
     ...
     * @return Const reference to map of active pointer IDs to their addresses
     */
    static const std::unordered_map<int, const void*>& GetActiveAptrs()
    {
