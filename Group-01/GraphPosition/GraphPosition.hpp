 ...
 * @return Vector of pairs containing edge ID and edge pointer, sorted by
 * destination vertex ID
 */
template <typename VERTEX_DATA_T>
auto GetSortedNeighbors(const Vertex<VERTEX_DATA_T> &vertex) {
  using EdgePair = std::pair<std::string, std::weak_ptr<Edge<VERTEX_DATA_T>>>;
  std::vector<EdgePair> neighbors(vertex.GetEdges().begin(),
                                  vertex.GetEdges().end());

  std::sort(neighbors.begin(), neighbors.end(),
            [](const auto &edge1, const auto &edge2) {
              return edge1.second.lock()->GetTo().GetId() <
                     edge2.second.lock()->GetTo().GetId();
            });
  return neighbors;
}
...
template <typename VERTEX_DATA_T>
auto DFS()
    -> std::function<bool(GraphPosition<VERTEX_DATA_T> &)>;  // Forward declare
                                                             // the function
...
class GraphPosition {
 ...
 private:
  ...
  std::function<bool(GraphPosition<VERTEX_DATA_T> &)> traversalFunction =
      TraversalModes::DFS<VERTEX_DATA_T>();
 ...
 public:
  ...
  GraphPosition(const Graph<VERTEX_DATA_T> &g,
                Vertex<VERTEX_DATA_T> const &startVertex);
   ...
   */
  std::vector<Vertex<VERTEX_DATA_T> const *> &GetTraversalStack() {
   ...
   */
  std::deque<Vertex<VERTEX_DATA_T> const *> &GetTraversalQueue() {
    return traversalQueue;
  }

  /**
   * Sets the traversal strategy
   * @param newTraversalFunction Function implementing the desired traversal
   * strategy
   */
  void SetTraversalMode(std::function<bool(GraphPosition<VERTEX_DATA_T> &)>
                            newTraversalFunction) {
   ...
   */
  cse::Vertex<VERTEX_DATA_T> &GetVertex(std::string const v_id) {
    return graph.GetVertex(v_id);
  }
};
 ...
 */
template <typename VERTEX_DATA_T>
void GraphPosition<VERTEX_DATA_T>::MarkVisited(
 ...
 */
template <typename VERTEX_DATA_T>
Vertex<VERTEX_DATA_T> const &GraphPosition<VERTEX_DATA_T>::GetCurrentVertex()
    const {
  assert(currentVertex != nullptr &&
         "GetCurrentVertex() should never return nullptr!");
 ...
 */
template <typename VERTEX_DATA_T>
const std::vector<Vertex<VERTEX_DATA_T> const *> &
GraphPosition<VERTEX_DATA_T>::GetTraversalPath() const {
 ...
 */
template <typename VERTEX_DATA_T>
void GraphPosition<VERTEX_DATA_T>::ResetTraversalPath() {
  traversalPath.clear();
}
 ...
 */
template <typename VERTEX_DATA_T>
void GraphPosition<VERTEX_DATA_T>::AddToTraversalPath(
 ...
 */
template <typename VERTEX_DATA_T>
void GraphPosition<VERTEX_DATA_T>::ReverseTraversalPath() {
 ...
 */
template <typename VERTEX_DATA_T>
void GraphPosition<VERTEX_DATA_T>::ResetTraversal(
    Vertex<VERTEX_DATA_T> const &newStartVertex) {
...
/**
 * Traverses the graph using the provided traversal strategy until no next node
 * available
 */
template <typename VERTEX_DATA_T>
inline void GraphPosition<VERTEX_DATA_T>::TraverseGraph() {
  while ((bool)++(*this)) {
  }
}
...
auto DFS() -> std::function<bool(GraphPosition<VERTEX_DATA_T> &)> {
  return [](GraphPosition<VERTEX_DATA_T> &graphPosition) {
    ...
    auto dfs_implementation = [&](GraphPosition<VERTEX_DATA_T> &gp,
                                  auto &dfs) -> bool {
      ...
      auto nonVisited =
          std::find_if(neighbors.begin(), neighbors.end(), [&](auto &p) {
...
auto BFS() -> std::function<bool(GraphPosition<VERTEX_DATA_T> &)> {
  return [](GraphPosition<VERTEX_DATA_T> &graphPosition) {
    ...
    std::vector<std::pair<std::string, std::weak_ptr<Edge<VERTEX_DATA_T>>>>
        neighbors(current->GetEdges().begin(), current->GetEdges().end());
    std::sort(neighbors.begin(), neighbors.end(),
              [](const auto &edge1, const auto &edge2) {
                return edge1.second.lock()->GetTo().GetId() <
                       edge2.second.lock()->GetTo().GetId();
 ...
 */
template <typename VERTEX_DATA_T>
struct VectorDistancePair {
  Vertex<VERTEX_DATA_T> const *v;
  double distance;
  VectorDistancePair(Vertex<VERTEX_DATA_T> const *vertex, double d)
      : v(vertex), distance(d) {}
};
 ...
 */
template <typename VERTEX_DATA_T>
bool operator<(const VectorDistancePair<VERTEX_DATA_T> &lhs,
               const VectorDistancePair<VERTEX_DATA_T> &rhs) {
...
template <typename VERTEX_DATA_T>
auto AStar(Vertex<VERTEX_DATA_T> &destination)
    -> std::function<bool(GraphPosition<VERTEX_DATA_T> &)> {
  using VertexPair = VectorDistancePair<VERTEX_DATA_T>;
  using PriorityQueue = std::priority_queue<VertexPair, std::vector<VertexPair>,
  ...
  return [q, distanceCalculator, bestDistances,
          &destination](GraphPosition<VERTEX_DATA_T> &graphPosition) mutable {
    if (q.empty()) {
