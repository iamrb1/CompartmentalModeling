...
class Distribution {
 private:
  ...
  std::vector<double> cumulativeProbs;  // Holds the cumulative probabilities of
                                        // the distribution
  const double noChance =
      0.0;  // A value for when the probability is none or "0"
  const double maxProb = 1.0;  // The maximum probability
  const int minTrials = 1;     // The minimum number of trials you can build
  const int minTrialsCheck =
      0;  // The minimum number of trials to check if successful
...
void Distribution::BuildBinomial(const int trials, const double prob) {
  if (trials < minTrials) {
    throw std::runtime_error(
        "Must have at least 1 datapoint");  // Use the cse::assert in future
  }
  if (prob > maxProb || prob < noChance) {
    throw std::out_of_range(
        "Probability not expessed as a decimal.");  // Use the cse::assert in
                                                    // future
  }
  ...
  for (int i = 0; i < trials; i++) {
    // The formula for binomial probability and populate the probabilities into
    // probs
    probs[i] = std::tgamma(trials + 1.0) /
               (std::tgamma(i + 1.0) * std::tgamma(trials - i + 1.0)) *
...
double Distribution::ProbCalculator(const std::vector<double> probVector,
                                    const int trials) {
