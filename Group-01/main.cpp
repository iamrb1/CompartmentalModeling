...
private:
        ...
        EM_ASM_(
            {
              ...
              ctx.fillText($0.toString(), $1, $2);
            },
            edge->GetWeight(), midX, midY);
      }
    }
  ...
  // ChatGPT used to create display section
  void CreateDisplayOptionsSection() {
    EM_ASM({
      var container = document.createElement('div');
      container.classList.add("control-section-container");

      var header = document.createElement('h2');
      header.innerHTML = "Display Options";
      ...
      displayGroup.classList.add("button-group");
      displayGroup.style.padding = "20px";

      // Show Vertex IDs toggle
      var showVertexIdsContainer = document.createElement('div');
      showVertexIdsContainer.style.display = "flex";
      showVertexIdsContainer.style.alignItems = "center";
      showVertexIdsContainer.style.marginBottom = "15px";
      showVertexIdsContainer.style.justifyContent = "space-between";
      showVertexIdsContainer.style.padding = "0 20px";

      var showVertexIdsLabel = document.createElement('label');
      showVertexIdsLabel.htmlFor = 'showVertexIdsToggle';
      showVertexIdsLabel.textContent = 'Show Vertex IDs';
      showVertexIdsLabel.style.fontSize = "16px";

      var showVertexIdsCheckbox = document.createElement('input');
      showVertexIdsCheckbox.type = 'checkbox';
      showVertexIdsCheckbox.id = 'showVertexIdsToggle';
      showVertexIdsCheckbox.checked = true;

      showVertexIdsContainer.appendChild(showVertexIdsLabel);
      showVertexIdsContainer.appendChild(showVertexIdsCheckbox);

      // Show Edge Weights toggle
      var showEdgeWeightsContainer = document.createElement('div');
      showEdgeWeightsContainer.style.display = "flex";
      showEdgeWeightsContainer.style.alignItems = "center";
      showEdgeWeightsContainer.style.justifyContent = "space-between";
      showEdgeWeightsContainer.style.padding = "0 20px";

      var showEdgeWeightsLabel = document.createElement('label');
      showEdgeWeightsLabel.htmlFor = 'showEdgeWeightsToggle';
      showEdgeWeightsLabel.textContent = 'Show Edge Weights';
      showEdgeWeightsLabel.style.fontSize = "16px";

      var showEdgeWeightsCheckbox = document.createElement('input');
      showEdgeWeightsCheckbox.type = 'checkbox';
      showEdgeWeightsCheckbox.id = 'showEdgeWeightsToggle';
      showEdgeWeightsCheckbox.checked = true;

      showEdgeWeightsContainer.appendChild(showEdgeWeightsLabel);
      ...
      optionsDiv.appendChild(displayGroup);

      container.appendChild(header);
      container.appendChild(optionsDiv);

      window.displayOptionsSection = container;
    });
  }
  ...
  void CreateGraphGenerationSection() {
    EM_ASM({
      ...
      useSampleButton.addEventListener(
          'click', function() {
            var selected = document.getElementById("sampleGraphSelect").value;
            switch (selected) {
              case "DefaultGraph":
                Module._chooseSampleGraph(0);
                break;
              case "CycleGraph":
                Module._chooseSampleGraph(1);
                break;
              case "CompleteGraph":
                Module._chooseSampleGraph(2);
                break;
              case "BinaryTree":
                Module._chooseSampleGraph(3);
                break;
              case "ButterflyGraph":
                Module._chooseSampleGraph(4);
                break;
              case "GridGraph":
                Module._chooseSampleGraph(5);
                break;
            }
          });
  ...
  void InitializeControlZone() {
    EM_ASM({
      ...
      controlZone.appendChild(window.displayOptionsSection);
...
void chooseSampleGraph(int type) {
  switch (type) {
    case 0:
      init.ChooseSampleGraph(GraphFactory::DefaultGraph);
      break;
    case 1:
      init.ChooseSampleGraph(GraphFactory::CycleGraph);
      break;
    case 2:
      init.ChooseSampleGraph(GraphFactory::CompleteGraph);
      break;
    case 3:
      init.ChooseSampleGraph(GraphFactory::BinaryTree);
      break;
    case 4:
      init.ChooseSampleGraph(GraphFactory::ButterflyGraph);
      break;
    case 5:
      init.ChooseSampleGraph(GraphFactory::GridGraph);
      break;
  }
}
}
