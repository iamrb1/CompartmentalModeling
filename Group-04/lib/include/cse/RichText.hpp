...
namespace cse {
    ...
    public:
        struct Format{
            ...
            Format(std::string name, std::variant<std::string, int32_t, std::monostate> metadata) : name(std::move(name)), metadata(std::move(metadata)) {}
            Format(std::string name) : name(std::move(name)), metadata(std::monostate()) {}
    ...
    public:
        ...
        RichText& append(const RichText& str)
        {
            ...
            while (left != left_end && right != right_end) {
                ...
                formatting.insert({right->first, right->second}).first->second.offset(text.size());
                ++right;
            }
            text += str.text;
            return *this;
        ...
        void applyFormatToRange(const Format& format, const size_t begin, const size_t end)
        {
