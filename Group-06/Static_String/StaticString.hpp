...
private:
    ...
    char mString[MaxSize + 1]; // Array of characters that stores our string with '\0' terminator at the end.
...
public:
     ...
     */
    StaticString() : currentSize(0) {
        mString[0] = '\0';
    }


    // Currently Static String stored on stack memory, We might include another version supports heap allocation too.
    ...
    StaticString(const char* cstr) : currentSize(0) {
        ...
        if (cstr[currentSize] != '\0') {
            ...
            throw std::length_error("Input string exceeds StaticString defined maximum size.");
        }
    ...
    /**
     * @brief Construct a new Static String object from copying another Static String object.
     * 
     * @param staticString Static String object to be coppied.
     */
    StaticString(const StaticString& staticString) : currentSize(staticString.currentSize) {
        for (std::size_t i = 0; i <= currentSize; ++i)
        {
            mString[i] = staticString.mString[i];
        }
    }
    ...
    char& operator[](std::size_t index) { 
        ...
        {
            throw std::out_of_range("Indexed value is out of range, Index = " + std::to_string(index));
        }
