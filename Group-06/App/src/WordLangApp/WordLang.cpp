  ...
  switch (mTokenManager.Peek()) {
    ...
    default: {
      mErrorManager.printInfo(
          "Incorrect Syntax: Check syntax, keyword is not found.");
      break;
    }
  }
}
...
void cse::WordLang::parsePrint() {
  ...
  if (identifier.id == 236) {  // PRINT ALL
    ...
    mErrorManager.printInfo(
        "Incorrect Syntax: Incorrect identifier type, must be a number or "
        "\"ALL\"");
    return;
  }
...
void cse::WordLang::parseAdd() {
  ...
  // Syntax is correct, call WordListManager to handle. WIth file name and list
  // name to load
  std::string trimmedWords = words.lexeme.substr(1, words.lexeme.length() - 2);

  // Call WordListManager to handle with listname, trimmedWords to add into
  // list.
...
void cse::WordLang::parseLength() {
  ...
  if (length == mTokenManager.eof_token ||
    ...
    // Print syntax error
    mErrorManager.printInfo(
        "Incorrect Syntax: Length value must be number or \"*\"");
    return;
  }
...
void cse::WordLang::parseContainsAny() {
  ...
  if (letters == mTokenManager.eof_token ||
    ...
    // Print syntax error
    mErrorManager.printInfo(
        "Incorrect Syntax: CONTAINS_ANY must have string of letters.");
    return;
  }
  ...
  std::string trimmedLetters =
      letters.lexeme.substr(1, letters.lexeme.length() - 2);
...
void cse::WordLang::parseContainsAll() {
  ...
  if (letters == mTokenManager.eof_token ||
    ...
    mErrorManager.printInfo(
        "Incorrect Syntax: CONTAINS_ALL must have string of letters.");
    return;
  }
...
void cse::WordLang::parseNotContains() {
  ...
  if (letters == mTokenManager.eof_token ||
    ...
    // Print error and return
    mErrorManager.printInfo(
        "Incorrect Syntax: NOT_CONTAINS must have string of letters.");
    return;
  }
...
void cse::WordLang::parseGet() {
  ...
  if (letters == mTokenManager.eof_token ||
    ...
    // Print error and return
    mErrorManager.printInfo(
        "Incorrect Syntax: GET must have string of letters or wildcards.");
    return;
  }
...
void cse::WordLang::parseReset() {
  ...
  mErrorManager.printInfo(
      "Incorrect Syntax: encountered unknown symbols in RESET statement");
}
...
void cse::WordLang::parseWordle() {
  ...
  std::string trimmedResult =
      result.lexeme.substr(1, result.lexeme.length() - 2);
