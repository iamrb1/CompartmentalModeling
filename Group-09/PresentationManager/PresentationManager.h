...
class PresentationManager {
 public:  // <- all pub for testing // TODO: change back to private
  ...
  // The WebLayoutManager pointer is the first argument for the static functions
  // that change the presentation and are called inside of the events.
  ...
  // We create static functions that can be passed to the EventManager (and the
  // Event) The first argument is a pointer to the layoutManager, the second is
  // an ID (slide number, text box id, etc.)
  static void changeSlide(std::shared_ptr<WebLayoutManager> layoutManager,
                          size_t slideNum) {
...
int main() {
  ...
  manager.eventManager.AddEvent(1, manager.changeSlide, manager.layoutManager,
                                5);
  return 0;
}
