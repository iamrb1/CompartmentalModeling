...
void WebLayout::PushTextBox(const std::string& msg, const int& width, const int& height)
{
    EM_ASM({
        ...
        if (textBoxDiv)
        {
            textBoxDiv.innerHTML += '<p style="border: 1px solid black; border-radius: 5px; height: ' + height + 'px; width: ' + width + 'px;">' + msg + '</p>';
        }
 ...
 * @param yLoc of image
 */
void WebLayout::PushImage(const std::string& url, const int& width, const int& height, const int& xLoc, const int& yLoc)
{
    EM_ASM({
        var msg = UTF8ToString($0);
        var width = $1;
        ...
        if (imageBoxDiv)
        {
            imageBoxDiv.innerHTML += "<img src='" + msg + "' style='object-fit: fill; width:" + width + "px; height:" + height +"; left: " + xLoc + "px; top: " + yLoc + "px;' />";
        }
